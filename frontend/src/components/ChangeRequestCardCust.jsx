                    ...
                    {hasChanges ? (
                        changeLogs.map((log, index) => (
                            <div key={index} className="mb-4">
                                ...
                                {renderChange(log, 'Cost', 'Cost')}
                                <div className="mt-3">
                                    <button onClick={() => handleApproval(log.customerId, log.projectId, log.phaseId, log._id, 'Approved', token)}>Approve</button>
                                    <button onClick={() => handleRejection(log.customerId, log.projectId, log.phaseId, log._id, 'Reject', token)}>Reject</button>
                                </div>
                            </div>
                        ))
                    ) : (
